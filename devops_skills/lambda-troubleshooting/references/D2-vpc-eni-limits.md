---
title: "D2 — VPC ENI Limits"
description: "Diagnose Lambda ENI creation failures in VPC"
status: active
severity: MEDIUM
triggers:
...
owner: devops-agent
objective: "Resolve ENI limit issues for VPC Lambda"
context: "Lambda uses Hyperplane ENIs (shared across functions with same subnet+SG combo). ENI limits are rarely hit with Hyperplane but can occur with many unique subnet/SG combinations or in accounts with many VPC resources."
---

## Common Issues

- symptoms: "ENILimitReachedException"
  diagnosis: "Account/region ENI limit reached. Lambda shares ENI quota with EC2, ECS, etc."
  resolution: "Request ENI limit increase. Reduce unique subnet/SG combinations. Clean up unused ENIs."
...
```yaml
root_cause: "<eni_limit|too_many_subnet_sg_combos>"
severity: MEDIUM
mitigation:
  immediate: "Request ENI limit increase, consolidate subnet/SG combos"
  long_term: "Minimize unique VPC configurations across Lambda functions"
```
